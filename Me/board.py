...
class GoBoard(object):
    ...
    def is_legal(self, point, color):
        ...
        return self.check_legal(point, color)
    ...
    def precompute_neighbors(self):
        for m in where1d(self.board == EMPTY):
            self.moves[m] = [m - 1, m + 1, m - self.NS, m + self.NS]
        for m in where1d(self.board == BLACK):
            self.moves[m] = [m - 1, m + 1, m - self.NS, m + self.NS]
        for m in where1d(self.board == WHITE):
            self.moves[m] = [m - 1, m + 1, m - self.NS, m + self.NS]

    def check_legal(self, point, color) -> bool:
        """
        Play a move of color on point
